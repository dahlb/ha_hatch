...
class HatchDataUpdateCoordinator(DataUpdateCoordinator[dict]):
    ...
    async def _async_update_data(self) -> List[dict]:
        ...
        _LOGGER.debug(
            f"credentials expire at: {datetime.datetime.fromtimestamp(self.expiration_time, datetime.UTC)}"
        )
        self.update_interval = datetime.datetime.fromtimestamp(self.expiration_time - 60, datetime.UTC) - datetime.datetime.now(datetime.UTC)
        for rest_device in self.rest_devices:
